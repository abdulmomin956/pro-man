    ...
    return (
        <div className='sm:w-full lg:w-1/2 shadow py-12 shadow-primary mx-auto px-12'>
            ...
            <form action="#">
                ...
                <input required type="text" disabled placeholder={user?.displayName} className="w-full border px-3 my-2 py-1  text-black" />
                <label htmlFor="">Email Address</label> <br />
                <input required type="text" disabled placeholder={user?.email} className="w-full border px-3 my-2 py-1 text-black" />
                ...
                <div onClick={() => setList(!list)} className='w-full border bg-gray-50 flex justify-between px-3 my-2 py-1'>
                    <input required type="text" disabled placeholder={selectItem ? `${selectItem}` : 'Select from the list'} className=" bg-transparent text-black" />
                ...
                {(selectItem === "Using Proman") &&
                    <div onClick={() => setupList(!upList)} className='w-full border bg-gray-50 flex justify-between px-3 my-2 py-1'>
                        <input required type="text" disabled placeholder={upTopic ? `${upTopic}` : 'Select from the list'} className=" bg-transparent text-black" />
                        <svg className="fill-current" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"><path d="M7.41,8.58L12,13.17L16.59,8.58L18,10L12,16L6,10L7.41,8.58Z" /></svg>
                    </div>}
                {upList && <div>
                ...
                {(selectItem === "Acounts Issue") &&
                    <div onClick={() => setAcList(!acList)} className='w-full border bg-gray-50 flex justify-between px-3 my-2 py-1'>
                        <input required type="text" disabled placeholder={acTopic ? `${acTopic}` : 'Select from the list'} className=" bg-transparent text-black" />
                        <svg className="fill-current" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"><path d="M7.41,8.58L12,13.17L16.59,8.58L18,10L12,16L6,10L7.41,8.58Z" /></svg>
                    </div>}
                {acList && <div>
